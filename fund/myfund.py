...
class Fund:
    def __init__(self, fundNo, buyPrice=0, buyCount=0, isRMB="", buyDate=''):
        self.fundNo = fundNo
    ...
    def newPrice2Tr(self):
        ...
        ret += '<td>' + '%.2f' % ((float(self.nowPrice)-float(self.buyPrice))/float(self.buyPrice)*100) +'%' + '</td>\n'
        ret += '<td>' + self.nowDate + '</td>\n'
        ret += '<td>' + self.isRMB + '%.2f' % ((float(self.nowPrice)-float(self.buyPrice))*float(self.buyCount)) + '</td>\n'
        ret += '<td>' + self.nowPrice + '</td>\n'
        ret += '<td>' + self.buyPrice + '</td>\n'
        ret += '<td>' + self.buyCount + '</td>\n'
...
def getmyfund():
    funds = []
    funds.append(Fund('002402', '0.162', '30867', '$', '16-10-18'))
    funds.append(Fund('003721', '0.1614', '278731', '$', '17-05-12'))
    funds.append(Fund('164906', '1.2432', '329790', '', '17-05-22'))
    for f in funds:
        f.getNewPrice()
        
    ret = '<html><title>fund</title>%s\n' % HTMLTABLE
    ret += '<tr><td>name</td><td>%</td><td>date</td><td>$</td><td>Now$</td><td>Buy$</td><td>count</td><td>start</td></tr>'
    for f in funds:
        ret += f.newPrice2Tr()
    ret += '</table>\n'
    ret += 'ver:170804-1'
